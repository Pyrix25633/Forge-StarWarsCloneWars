...
public class BlasterDc15sItem extends Item {
	...
	public void onUsingTick(ItemStack itemstack, LivingEntity entityLiving, int count) {
		...
		if (!world.isClientSide() && entityLiving instanceof ServerPlayer entity) {
			...
			if (true) {
				ItemStack stack = ProjectileWeaponItem.getHeldProjectile(entity, e -> e.getItem() == StarWarsModItems.BLASTER_MUNITIONS.get());
				if (stack == ItemStack.EMPTY) {
					for (int i = 0; i < entity.getInventory().items.size(); i++) {
						ItemStack teststack = entity.getInventory().items.get(i);
						if (teststack != null && teststack.getItem() == StarWarsModItems.BLASTER_MUNITIONS.get()) {
							stack = teststack;
							break;
						}
					}
				...
				if (entity.getAbilities().instabuild || stack != ItemStack.EMPTY) {
					BlasterDc15sEntity entityarrow = BlasterDc15sEntity.shoot(world, entity, world.getRandom(), 6f, 4.5, 0);
					itemstack.hurtAndBreak(1, entity, e -> e.broadcastBreakEvent(entity.getUsedItemHand()));
					if (entity.getAbilities().instabuild) {
						entityarrow.pickup = AbstractArrow.Pickup.CREATIVE_ONLY;
					} else {
						if (new ItemStack(StarWarsModItems.BLASTER_MUNITIONS.get()).isDamageableItem()) {
