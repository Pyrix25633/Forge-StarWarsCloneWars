...
public class BlasterDc17Item extends Item {
	...
	public void releaseUsing(ItemStack itemstack, Level world, LivingEntity entity, int time) {
		if (!world.isClientSide() && entity instanceof ServerPlayer player) {
			ItemStack stack = ProjectileWeaponItem.getHeldProjectile(entity, e -> e.getItem() == BlasterDc17ProjectileEntity.PROJECTILE_ITEM.getItem());
			if (stack == ItemStack.EMPTY) {
				for (int i = 0; i < player.getInventory().items.size(); i++) {
					ItemStack teststack = player.getInventory().items.get(i);
					if (teststack != null && teststack.getItem() == BlasterDc17ProjectileEntity.PROJECTILE_ITEM.getItem()) {
						stack = teststack;
						break;
					}
				}
			...
			if (player.getAbilities().instabuild || stack != ItemStack.EMPTY) {
				BlasterDc17ProjectileEntity projectile = BlasterDc17ProjectileEntity.shoot(world, entity, world.getRandom());
